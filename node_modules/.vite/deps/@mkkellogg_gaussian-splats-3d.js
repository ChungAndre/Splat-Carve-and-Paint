import { $r as MathUtils, As as UnsignedByteType, Bn as HalfFloatType, Bo as SphereGeometry, Bs as Vector3, Da as RGBAIntegerFormat, Do as Scene, Ea as RGBAFormat, G as BoxGeometry, J as BufferGeometry, Js as WebGLRenderTarget, Lt as CylinderGeometry, Oo as ShaderMaterial, Ps as UnsignedIntType, Qi as OrthographicCamera, Rn as Group, U as Box3, Ut as DataUtils, Vi as Object3D, Vo as Spherical, Vs as Vector4, Vt as DataTexture, Xn as InstancedBufferGeometry, Yn as InstancedBufferAttribute, Yr as MOUSE, Yt as DepthTexture, _a as Quaternion, ai as MeshBasicMaterial, an as DynamicDrawUsage, hn as EventDispatcher, ht as ConeGeometry, ia as PlaneGeometry, ii as Mesh, lo as Ray$1, na as PerspectiveCamera, ni as Matrix4, o as WebGLRenderer, oo as RGFormat, po as RedIntegerFormat, q as BufferAttribute, qt as DepthFormat, ra as Plane, s as WebGLUtils, ss as TOUCH, st as Color, ti as Matrix3, xn as FloatType, zs as Vector2 } from "./three.module-slveYrFH.js";
...
var convertBetweenCompressionLevels = function() {
	...
	return function(val, fromLevel, toLevel, isSH = false) {
		...
		if (fromLevel === 2 && isSH) {
			...
			else if (toLevel == 2) if (!isSH) outputConversionFunc = noop;
			else outputConversionFunc = fromHalfFloatToUint8;
		} else if (fromLevel === 0) {
			if (toLevel === 1) outputConversionFunc = toHalfFloat;
			else if (toLevel == 2) if (!isSH) outputConversionFunc = toHalfFloat;
...
var PlyParserUtils = class PlyParserUtils {
	...
	static decodeSectionHeader(headerLines, fieldNameIdMap, headerStartLine = 0) {
		...
		for (let i = headerStartLine; i < headerLines.length; i++) {
			...
			if (line.startsWith("element")) if (processingSection) {
				headerEndLine--;
				break;
			} else {
	...
	static extractHeaderFromBufferToText(plyBuffer) {
		...
		while (true) {
			...
			if (PlyParserUtils.checkBufferForEndHeader(plyBuffer, headerOffset, readChunkSize * 2, decoder)) break;
		}
		return headerText;
	}
	...
	static readVertex(vertexData, header, row, dataOffset, fieldsToRead, rawVertex, normalize = true) {
		...
		for (let fieldId of fieldsToRead) {
			...
			else if (fieldType === FieldSizeIdUChar) if (normalize) rawVertex[fieldId] = vertexData.getUint8(offset + fieldOffsets[fieldId]) / 255;
...
var fromHalfFloat = (hf) => {
	const t = (31744 & hf) >> 10;
	const a = 1023 & hf;
	return (hf >> 15 ? -1 : 1) * (t ? t === 31 ? a ? NaN : Infinity : Math.pow(2, t - 15) * (1 + a / 1024) : a / 1024 * 6103515625e-14);
};
...
var SplatBufferGenerator = class SplatBufferGenerator {
	...
	static getStandardGenerator(alphaRemovalThreshold = 1, compressionLevel = 1, sectionSize = 0, sceneCenter = new Vector3(), blockSize = SplatBuffer.BucketBlockSize, bucketSize = SplatBuffer.BucketSize) {
		return new SplatBufferGenerator(SplatPartitioner.getStandardPartitioner(sectionSize, sceneCenter, blockSize, bucketSize), alphaRemovalThreshold, compressionLevel, sectionSize, sceneCenter, blockSize, bucketSize);
	}
};
var LoaderStatus = {
	"Downloading": 0,
	...
	static loadFromURL(fileName, onProgress, progressiveLoadToSplatBuffer, onProgressiveLoadSectionProgress, minimumAlpha, compressionLevel, optimizeSplatData = true, outSphericalHarmonicsDegree = 0, headers, sectionSize, sceneCenter, blockSize, bucketSize) {
		...
		const localOnProgress = (percent, percentLabel, chunkData) => {
			...
			if (internalLoadType === InternalLoadType.DownloadBeforeProcessing) {
				...
				if (!headerLoaded) {
					...
					if (PlyParserUtils.checkTextForEndHeader(headerText)) {
						...
						if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) {
							const splatBufferSizeBytes = splatBufferDataOffsetBytes + SplatBuffer.CompressionLevels[0].SphericalHarmonicsDegrees[outSphericalHarmonicsDegree].BytesPerSplat * maxSplatCount;
				...
				if (loadComplete) if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) loadPromise.resolve(directLoadSplatBuffer);
...
var SplatLoader = class SplatLoader {
	static loadFromURL(fileName, onProgress, progressiveLoadToSplatBuffer, onProgressiveLoadSectionProgress, minimumAlpha, compressionLevel, optimizeSplatData = true, headers, sectionSize, sceneCenter, blockSize, bucketSize) {
		...
		const localOnProgress = (percent, percentStr, chunk, fileSize) => {
			...
			if (!fileSize) if (progressiveLoadToSplatBuffer) throw new DirectLoadError("Cannon directly load .splat because no file size info is available.");
			else {
				internalLoadType = InternalLoadType.DownloadBeforeProcessing;
				return;
			}
			if (!directLoadBufferIn) {
				...
				const splatBufferSizeBytes = splatDataOffsetBytes + SplatBuffer.CompressionLevels[0].SphericalHarmonicsDegrees[0].BytesPerSplat * maxSplatCount;
				if (internalLoadType === InternalLoadType.ProgressiveToSplatBuffer) {
					directLoadBufferOut = new ArrayBuffer(splatBufferSizeBytes);
					SplatBuffer.writeHeaderToBuffer({
						versionMajor: SplatBuffer.CurrentMajorVersion,
...
var SplatGeometry = class {
	...
	*/
	static build(maxSplatCount) {
		...
		const splatIndexes = new InstancedBufferAttribute(new Uint32Array(maxSplatCount), 1, false);
...
var SplatMesh = class SplatMesh extends Mesh {
	...
	*/
	...
	freeIntermediateSplatData() {
		...
		if (this.splatDataTextures.sphericalHarmonics) if (this.splatDataTextures.sphericalHarmonics.texture) {
...
var Viewer = class Viewer {
	...
	init() {
		...
		if (!this.rootElement) if (!this.usingExternalRenderer) {
		...
		return function(e) {
			...
			switch (e.code) {
				...
				case "Equal":
					...
					break;
				case "Minus":
					if (!this.usingExternalCamera) this.splatMesh.setSplatScale(Math.max(this.splatMesh.getSplatScale() - .05, 0));
					break;
			}
		};
	}();
	...
	*/
	addSplatScene(path, options = {}) {
		...
		const onProgressUIUpdate = (percentComplete, percentCompleteLabel, loaderStatus) => {
			if (showLoadingUI) {
				if (loaderStatus === LoaderStatus.Downloading) if (percentComplete == 100) this.loadingSpinner.setMessageForTask(loadingUITaskId, "Download complete!");
		...
		const splatBuffersAddedUIUpdate = (firstBuild, finalBuild) => {
			if (showLoadingUI) {
				...
				if (progressiveLoad) if (finalBuild) {
					downloadDone = true;
					this.loadingProgressBar.hide();
	...
	runSplatSort = function() {
		...
		return function(force = false, forceSortAll = false) {
			...
			gpuAcceleratedSortPromise.then(() => {
				if (queuedSorts.length === 0) if (this.splatMesh.dynamicMode || shouldSortAll) queuedSorts.push(this.splatRenderCount);
				else {
